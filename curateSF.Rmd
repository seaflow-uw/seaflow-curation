...
params_yaml <- "params:
  db: HOT326.vct.db
  curation_config_path: HOT326.curation_params.tsv
  save_path: HOT326.curation_outliers.tsv
  fig_width: 8
  fig_height: 4
  warning: false
  message: false
...
if (nrow(df2) > 0) {
  tryCatch({
    ...
    message(paste("Error in smooth.spline for event rate:", e$message))
    model <<- NULL
  })
  if (!is.null(model)) {
    res <- residuals(model)
    ...
    print(paste("Skipping event rate outlier detection due to smooth.spline error"))
  }
}
```
...
if (nrow(df2) > 0) {
  tryCatch({
    ...
    message(paste("Error in smooth.spline for file writing issues:", e$message))
    model <<- NULL
  })
  if (!is.null(model)) {
    res <- residuals(model)
    ...
    print(paste("Skipping file writing outlier detection due to smooth.spline error"))
  }
}
```
...
if (nrow(df2) > 0) {
  tryCatch({
    ...
    message(paste("Error in smooth.spline for OPP/EVT ratio:", e$message))
    model <<- NULL
  })
  if (!is.null(model)) {
    res <- residuals(model)
    ...
    print(paste("Skipping OPP/EVT ratio outlier detection due to smooth.spline error"))
  }
}
```
...
if (!is.na(curate$lim6_1)) {   # Some cruises don't gate for Pro
  ...
  if (nrow(df2) > 0) {
    tryCatch({
      ...
      message(paste("Error in smooth.spline for", phyto, ":", e$message))
      model <<- NULL
    })
    if (!is.null(model)) {
      res <- residuals(model)
      ...
      print(paste("Skipping Prochlorococcus size outlier detection due to smooth.spline error"))
    }
  }
} # End Pro conditional
```
...
if (nrow(df2) > 0) {
  ...
  if (!is.null(model)) {
    ...
    print(paste("Skipping Synechococcus size outlier detection due to smooth.spline error"))
  }
}
```
...
for (i in c(phyto)) {
  ...
  if (nrow(df1) > 0) {
    tryCatch({
      ...
      message(paste("Error in smooth.spline for", i, ":", e$message))
      model <<- NULL
    })
    ...
    if (!is.null(model)) {
      ...
      print(paste("Skipping abundance outlier detection for", i, "due to smooth.spline error"))
    }
  }
}
```
